...
- ベンダー画面で欠品投稿・推定取りこぼし売上・補充優先度を、期間/イベント/自販機/銘柄で絞り込んで確認し、CSVで書き出し
...
src/
  ...
  domain.ts         信頼度・鮮度判定・代替検索・並び順
  analytics.ts      ベンダー分析の集計とCSV出力
  api.ts            Worker APIクライアント
worker/             Cloudflare Worker(API・R2・AI解析・レート制限)
migrations/         D1のスキーマとサンプルデータ
data/               自販機・商品マスター、イベント、販売実測のCSV(D1へ取り込む正の情報)
...
- 期間(全期間 / 直近24時間 / 直近7日 / 登録イベントの会期)、自販機、銘柄で絞り込めます
- 欠品投稿の多い場所・銘柄・時間帯、買えなかった理由、その後の行動(コンビニへ移動・購入断念など)を集計します
- **推定値と実測値を列ごとに分けて表示します**。推定には必ず算出根拠(`欠品投稿 × 係数 × 商品単価`)を併記します
- 購買転換率の仮係数は画面上で変更でき、初期値は0.65です
- 実測がない間は「すべて推定」のバナーを常時表示します。実測は `data/sales_actuals.csv` に入れて `npm run db:import` で取り込みます
- 集計結果は「CSVで書き出す」でダウンロードできます(Excelで開けるようBOM付きUTF-8)

```text
data/events.csv         イベント会期(期間フィルターの選択肢になる)
data/sales_actuals.csv  販売・補充の実測値(machine_id,product_id,observed_on,units_sold,restock_units)
```
